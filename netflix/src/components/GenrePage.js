  ...
  return (
    <div>
      ...
      <div className="min-h-screen flex flex-col bg-gradient-to-b from-black via-neutral-900 to-black w-full">
        {/* Banner Section */}
        <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between px-2 sm:px-4 md:px-8 pt-20 pb-6 sm:pb-8 mb-2 sm:mb-4 gap-4">
          <div className="flex items-center gap-3 sm:gap-4 mb-2 md:mb-0">
            <div className="bg-gradient-to-tr from-red-600 to-yellow-400 rounded-full p-2 sm:p-3 shadow-lg">
              <FaFilm className="text-white text-2xl sm:text-3xl" />
            </div>
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-extrabold text-white drop-shadow-lg capitalize tracking-tight">
              {genreName?.replace(/-/g, ' ')}
            </h1>
          </div>
          {/* Genre Dropdown */}
          <div className="relative w-full max-w-xs md:max-w-xs md:w-48">
            ...
            <div className="pointer-events-none absolute inset-y-0 right-2 flex items-center text-yellow-400">
              <FaChevronDown className="w-4 h-4 sm:w-5 sm:h-5" />
            </div>
          </div>
        </div>
        <div className="flex-1 pb-8 sm:pb-10 px-2 sm:px-4 md:px-8">
          {/* Loading Spinner */}
          {loading && (
            <div className="flex justify-center items-center h-40 sm:h-64">
              <svg className="animate-spin h-8 w-8 sm:h-12 sm:w-12 text-red-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          {error && (
            <div className="flex flex-col items-center justify-center text-red-500 mt-8">
              <FaExclamationCircle className="text-3xl sm:text-4xl mb-2" />
              <span className="text-lg sm:text-xl font-semibold">{error}</span>
            </div>
          )}
          {/* Movie Grid */}
          ...
          {!loading && movies.length === 0 && !error && (
            <div className="flex flex-col items-center justify-center mt-10 sm:mt-16">
              <svg className="w-16 h-16 sm:w-24 sm:h-24 text-zinc-700 mb-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 48 48">
              ...
              <span className="text-white text-lg sm:text-xl font-semibold">No movies found for this genre.</span>
            </div>
          )}
        </div>
        <Footer />
